    ...
    <Layout>
      <section className="bg-slate-50 border-b py-12">
        <div className="container mx-auto px-4">
          <Link to="/products" className="inline-flex items-center text-slate-500 hover:text-industrial-blue mb-8 transition-colors">
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back to Products
          </Link>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className="text-4xl md:text-5xl font-bold text-industrial-blue mb-6">{category.title}</h1>
              <p className="text-xl text-slate-600 leading-relaxed mb-8">
                {category.description}
              </p>
              <div className="flex flex-wrap gap-4">
                <Button size="lg" className="bg-industrial-orange text-white hover:bg-industrial-orange/90">Request Datasheet</Button>
                <Button size="lg" variant="outline" className="border-industrial-blue text-industrial-blue">Technical Inquiry</Button>
              </div>
            </div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
            <div className="lg:col-span-2 space-y-12">
              <div>
                <h2 className="text-3xl font-bold text-industrial-blue mb-8">Product Range & Features</h2>
                  ...
                  {relatedProducts.length > 0 ? (
                    relatedProducts.map((product, i) => (
                      <motion.div
                        ...
                        className="p-6 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between group"
                      >
                        <span className="font-bold text-industrial-blue">{product.name}</span>
                        <ArrowRight className="w-4 h-4 text-tech-cyan group-hover:translate-x-1 transition-transform" />
                      </motion.div>
                    ))
                  ) : (
                    <div className="col-span-2 p-12 bg-slate-50 rounded-2xl text-center text-slate-400">
              ...
              <div>
                <h2 className="text-3xl font-bold text-industrial-blue mb-8">Why Choose Our {category.title}?</h2>
            ...
            <div className="lg:col-span-1 space-y-8">
              <motion.div 
                ...
                viewport={{ once: true }}
                className="bg-industrial-blue rounded-3xl p-8 text-white shadow-xl"
              >
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <button key={i} className="w-full flex items-center justify-between p-4 rounded-xl bg-white/10 hover:bg-white/20 transition-colors text-left group">
                      ...
                      <Download className="w-4 h-4 text-tech-cyan group-hover:scale-110 transition-transform" />
                    </button>
                  ))}
                </div>
                <Button className="w-full mt-8 bg-tech-cyan hover:bg-tech-cyan/90 text-white">Download All Resources</Button>
              </motion.div>
